...
class PromotionStatus(str, Enum):
    DRAFT = "draft"
    PENDING_APPROVAL = "pending_approval"  # Awaiting admin/leader approval
    SCHEDULED = "scheduled"
    ACTIVE = "active"
    PAUSED = "paused"
    ENDED = "ended"
...
class ApprovalRecord(BaseModel):
    ...
    status: Annotated[ApprovalStatus, Field(default=ApprovalStatus.PENDING, description="Approval status")]
    reviewer_id: Annotated[Optional[PydanticObjectId], Field(None, description="ID of the reviewer")]
    reviewer_type: Annotated[Optional[str], Field(None, description="Type: admin, leader, celebrity")]
    reviewed_at: Annotated[Optional[datetime], Field(None, description="When the review occurred")]
    notes: Annotated[Optional[str], Field(None, max_length=500, description="Reviewer notes")]


class ApprovalInfo(BaseModel):
    """Approval tracking for global and community visibility"""

    # Global feed approval (required for global/both visibility)
    global_approval: Annotated[Optional[ApprovalRecord], Field(None, description="Global feed approval")]

    # Per-community approvals (required for communities/both visibility)
    community_approvals: Annotated[Dict[str, ApprovalRecord], Field(
        default_factory=dict,
        description="Approval per community {community_id: ApprovalRecord}"
    )]

    def is_fully_approved(self, visibility: "PromotionVisibility") -> bool:
        """Check if all required approvals are complete"""
        if visibility.type in [VisibilityType.GLOBAL, VisibilityType.BOTH]:
            if not self.global_approval or self.global_approval.status != ApprovalStatus.APPROVED:
                return False

        if visibility.type in [VisibilityType.COMMUNITIES, VisibilityType.BOTH]:
            for cid in visibility.community_ids:
                cid_str = str(cid)
                if cid_str not in self.community_approvals:
                    return False
                if self.community_approvals[cid_str].status != ApprovalStatus.APPROVED:
                    return False

        return True

    def get_pending_approvals(self, visibility: "PromotionVisibility") -> List[str]:
        """Get list of pending approval scopes"""
        pending = []

        if visibility.type in [VisibilityType.GLOBAL, VisibilityType.BOTH]:
            if not self.global_approval or self.global_approval.status == ApprovalStatus.PENDING:
                pending.append("global")

        if visibility.type in [VisibilityType.COMMUNITIES, VisibilityType.BOTH]:
            for cid in visibility.community_ids:
                cid_str = str(cid)
                if cid_str not in self.community_approvals or \
                   self.community_approvals[cid_str].status == ApprovalStatus.PENDING:
                    pending.append(f"community:{cid_str}")
...
class RejectionRecord(BaseModel):
    ...
    scope: Annotated[str, Field(description="Scope: global or community:{id}")]
    reason: Annotated[str, Field(max_length=1000, description="Rejection reason")]
    reviewer_id: Annotated[PydanticObjectId, Field(description="ID of the reviewer")]
    reviewer_type: Annotated[str, Field(description="Type: admin, leader, celebrity")]
    rejected_at: Annotated[datetime, Field(default_factory=utc_now, description="Rejection timestamp")]
...
class Promotion(Document):
    ...
    # Approval workflow
    approval: Annotated[ApprovalInfo, Field(default_factory=ApprovalInfo, description="Approval tracking")]
    rejection_history: Annotated[List[RejectionRecord], Field(default_factory=list, description="History of rejections")]
    ...
    class Settings:
        ...
        indexes = [
            ...
            # Approval queue (pending approvals by visibility type)
    ...
    def can_delete(self) -> tuple:
        ...
        if self.status in [PromotionStatus.ACTIVE, PromotionStatus.PENDING_APPROVAL]:
    ...
    async def activate_promotion(self) -> None:
        """Activate a scheduled or approved promotion"""
        if self.status not in [PromotionStatus.SCHEDULED, PromotionStatus.PENDING_APPROVAL]:
            raise ValueError("Only scheduled or approved promotions can be activated")

        # Check if approvals are complete
        if not self.approval.is_fully_approved(self.visibility):
            pending = self.approval.get_pending_approvals(self.visibility)
            raise ValueError(f"Cannot activate: pending approvals for {pending}")
    ...
    async def submit_for_approval(self) -> None:
        """Submit promotion for approval (draft -> pending_approval)"""
        if self.status != PromotionStatus.DRAFT:
            raise ValueError("Only draft promotions can be submitted for approval")

        # Initialize approval records based on visibility
        if self.visibility.type in [VisibilityType.GLOBAL, VisibilityType.BOTH]:
            self.approval.global_approval = ApprovalRecord(status=ApprovalStatus.PENDING)

        if self.visibility.type in [VisibilityType.COMMUNITIES, VisibilityType.BOTH]:
            for cid in self.visibility.community_ids:
                cid_str = str(cid)
                self.approval.community_approvals[cid_str] = ApprovalRecord(status=ApprovalStatus.PENDING)

        self.status = PromotionStatus.PENDING_APPROVAL
        self.status_reasons.append("Submitted for approval")
        self.updated_at = utc_now()
        self.version += 1
        await self.save()
    ...
    async def approve(
        ...
        """
        Approve promotion for a specific scope.
        scope: 'global' or 'community'
        community_id: required if scope is 'community'
        """
        if self.status != PromotionStatus.PENDING_APPROVAL:
            raise ValueError("Only pending approval promotions can be approved")
        ...
        if scope == "global":
            if self.visibility.type not in [VisibilityType.GLOBAL, VisibilityType.BOTH]:
                raise ValueError("This promotion does not require global approval")
            self.approval.global_approval = approval_record
        elif scope == "community":
            if not community_id:
                raise ValueError("community_id required for community approval")
            if self.visibility.type not in [VisibilityType.COMMUNITIES, VisibilityType.BOTH]:
                raise ValueError("This promotion does not require community approval")
            if PydanticObjectId(community_id) not in self.visibility.community_ids:
                raise ValueError(f"Community {community_id} is not targeted by this promotion")
            self.approval.community_approvals[community_id] = approval_record
        else:
            raise ValueError(f"Invalid approval scope: {scope}")

        self.status_reasons.append(f"Approved by {reviewer_type} for {scope}")

        # Check if all approvals are complete
        if self.approval.is_fully_approved(self.visibility):
            # Transition to scheduled or active based on start_date
            ...
            if now >= start_date:
                ...
                if self.first_activated_at is None:
                    self.first_activated_at = now
                self.status_reasons.append("All approvals complete - activated immediately")
            else:
                self.status = PromotionStatus.SCHEDULED
                self.status_reasons.append("All approvals complete - scheduled")
    ...
    async def reject(
        ...
        """
        Reject promotion for a specific scope.
        Returns promotion to draft status with rejection recorded.
        """
        if self.status != PromotionStatus.PENDING_APPROVAL:
            raise ValueError("Only pending approval promotions can be rejected")

        now = utc_now()

        # Record rejection
        rejection_scope = scope if scope == "global" else f"community:{community_id}"
        ...
        self.rejection_history.append(rejection_record)

        # Update approval record
        if scope == "global":
            self.approval.global_approval = ApprovalRecord(
        ...
        elif scope == "community" and community_id:
            self.approval.community_approvals[community_id] = ApprovalRecord(
                status=ApprovalStatus.REJECTED,
                reviewer_id=reviewer_id,
                reviewer_type=reviewer_type,
                reviewed_at=now,
        ...
        # Return to draft
        self.status = PromotionStatus.DRAFT
        self.status_reasons.append(f"Rejected by {reviewer_type}: {reason}")
        self.updated_at = now
        self.version += 1
        await self.save()
